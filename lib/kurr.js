...
cpref = `Hayo Ngomongin Atsair Ganteng Yaaa Aowkowkwoo`
reply(cpref)
}
...
└─────────────────❒`
sendButLocation(from, `${menu}`, "*_© AtsairBotzゑ_*", {jpegThumbnail:ofrply2,name:""}, [{buttonId:`store`,buttonText:{displayText:'ATSAIRBOTZ STORE'},type:1},{buttonId:`sc`,buttonText:{displayText:'ＳＥＷＡ　ＢＯＴ'},type:1},{buttonId:`catalog`,buttonText:{displayText:'CATALOG MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              contentText:`Subrek yt : AtsrBotz`,buttons,headerType:4}
...
case 'memeindo':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ KurrXd', imageMessage: imageMsg,
              contentText:`Subrek yt : Atsr12`,buttons,headerType:4}
              prep = await kurr.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              kurr.relayWAMessage(prep)
                    break
...
sendButLocation(from, `${menu}`, "*_© Atsairゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'ALL MENU'},type:1},{buttonId:`store`,buttonText:{displayText:'ATSAIRBOT STORE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
gifnya = await getBuffer('https://telegra.ph/file/457d3be9baecef635c357.mp4')
		anu =`*AtsairBotz Melayani Jasa Jadi Bot+Owner*
*Dengan Harga Terjangkau*
*_List Harga Jadi Bot+Owner_*
1 MINGGU : 7K
...
INFO/SARAN: JANGAN PAKAI WA BUSINES
PAKAI WA ORI/WA DARI PLAYSTORE
...
MINAT HUB https://wa.me/6282393072006
`
...
ini_list.push({
"displayName": `Developer AtsairBotzゑ`,
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `AtsairBotzゑ`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
case 'chiisaihentai':
              ...
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ AtsairBotz', imageMessage: imageMsg,
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=KurrTod090909&text=${ini_txt}`).then((gambar) => {
                        kurr.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : AtsrBotz`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
...
case 'neko':
       ...
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await kurr.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ AtsairBot*, imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await kurr.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              kurr.relayWAMessage(prep)
